...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-path", default=f"{DEFAULT_OUTPUT_DIR}/stage_i_llama/mock_regression.json")
...
def main() -> None:
    ...
    for prompt in DEFAULT_PROMPTS:
        ...
        mapped_ids = map_input_ids(encoded["input_ids"], exported_perm)
        stage_logits_perm = exported_model(input_ids=mapped_ids, attention_mask=encoded["attention_mask"]).logits.detach().cpu().to(torch.float32)
        restored_logits = restore_logits(stage_logits_perm, exported_perm)

        baseline_generated_ids = greedy_generate_plain(baseline_model, encoded["input_ids"], args.max_new_tokens)[0].cpu()
        stage_generated_ids = greedy_generate_stage(exported_model, encoded["input_ids"], exported_perm, args.max_new_tokens)[0].cpu()
    ...
    payload = {
        "stage": "I",
        "variant": "llama_mock_stage_a_regression",
        "seed": args.seed,
    ...
    print(f"Saved mock Llama Stage-I regression report to {args.output_path}")
