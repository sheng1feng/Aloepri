...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run stage-B block0 attention fused-weight experiment.")
    parser.add_argument("--model-dir", default=DEFAULT_MODEL_DIR)
    parser.add_argument("--output-path", default=f"{DEFAULT_OUTPUT_DIR}/stage_b/block0_attn_fused.json")
...
def main() -> None:
    ...
    fused_stage_a_model = fuse_block0_attention_hidden_transform(stage_a_model, hidden_transform)

    baseline_recorder = TraceRecorder()
    observed_recorder = TraceRecorder()
    stage_b_model = StageBHiddenPermutationModel(
        stage_a_model=fused_stage_a_model,
    ...
    print(f"Saved stage-B block0 fused report to {args.output_path}")
