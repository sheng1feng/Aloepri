...
from src.security_qwen import build_sda_comparison_payload, default_frequency_gate4_targets, run_sda_baseline


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Gate-4 SDA across all knowledge settings and targets.")
    ...
    parser.add_argument("--output-path", default="outputs/security_qwen/summary/sda_comparison.json")
    return parser
...
def main() -> None:
    ...
    for knowledge_setting in ["zero_knowledge", "domain_aware", "distribution_aware"]:
        for target_name in args.targets:
            ...
            out = Path(f"outputs/security_qwen/sda/{target_name}.{knowledge_setting}.json")
            out.parent.mkdir(parents=True, exist_ok=True)
            out.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
            print(f"Saved SDA result to {out}")
    comparison = build_sda_comparison_payload(result_payloads=payloads)
    out = Path(args.output_path)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(comparison, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Saved SDA comparison to {out}")
