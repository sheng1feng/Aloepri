...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run remote validation for Llama baseline vs obfuscated checkpoint.")
    ...
    parser.add_argument("--output-path", default=f"{DEFAULT_OUTPUT_DIR}/llama_remote_validation.json")
...
def main() -> None:
    ...
    tokenizer, baseline_model = load_model_and_tokenizer(args.baseline_model_dir, device=args.device, dtype=args.dtype)
    bundle = load_stage_i_hf_bundle(args.server_dir, client_secret_path=args.client_secret, device=args.device, dtype=args.dtype)
    stage_model = bundle["model"]
    perm_vocab = bundle["perm_vocab"]

    prompt_results: list[dict] = []
    for prompt in DEFAULT_PROMPTS:
        encoded = tokenize_llama_prompt(tokenizer, prompt, device=args.device)
    ...
    print(f"Saved remote validation report to {args.output_path}")
