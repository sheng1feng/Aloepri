...
- KeyMat 体系的 fully-fused server-side 权重混淆版本
- KeyMat 与复杂 attention 的更高比例权重融合
- 论文更完整的动态 `Ẑ_block` 采样细节与更广泛消融
- 更贴近论文攻击面和安全评估的实验

因此，当前工作的准确定位应是：

> **从简化 AloePri 路线推进到 Algorithm 1 / KeyMat 原生体系的功能级复现与系统验证**
...
> **当前仓库已经完成 AloePri 技术报告的一条从“简化主线”推进到“原生 KeyMat 首次系统接入”的复现路线：从词表闭环，到 attention 子层恢复，到 block 完整恢复，再到多层传播验证、复杂 attention 结构接入修复，并进一步完成了论文 Algorithm 1 / KeyMat 体系的功能性接入。**
...
当前最合理的下一步不再是继续证明 KeyMat 能不能接进去，而是转向：
...
- 减少运行时显式 `Q/P` 投影
- 将 norm / attention / FFN 的 bridge 逐步融合进权重
- 评估 fused 后的数值稳定性与误差变化

这一步的目标是：

> 从“KeyMat 功能接入正确”推进到“KeyMat 更接近论文最终部署形式”。
