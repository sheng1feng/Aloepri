...
from src.keymat_embed_head import KeyMatEmbeddingWrapper, KeyMatHeadWrapper, add_head_noise, build_keymat_embed_head_artifacts
from src.keymat_norm import estimate_kappa_for_keymat
from src.stage_b import TraceRecorder, prepare_stage_a_model
from src.stage_f import LayerStageFConfig, _clear_copied_hooks, _ensure_list
from src.stage_g import KeyMatDecoderLayerHandoff
from src.stage_g_ffn import build_keymat_fused_ffn
from src.stage_g_norm import build_keymat_fused_rmsnorm
from src.stage_h_attention_static import build_staticized_attention


@dataclass(frozen=True)
class LayerStageHConfig:
...
class StageHModel(nn.Module):
    def __init__(
        self,
        stage_a_model,
        keymat_transform: KeyMatTransform,
        recorder: TraceRecorder | None,
        layer_configs: dict[int, LayerStageHConfig],
        ...
        for layer_idx in self.adapted_layers:
            ...
            layer.self_attn = build_staticized_attention(
...
def build_stage_h_model(
    ...
    if adapted_layers and adapted_layers != list(range(max(adapted_layers) + 1)):
        raise ValueError("Stage-H currently supports only contiguous prefix adapted layers.")
